...
class CaseFormManager(QtCore.QObject):
    ...
    def __init__(self, form, temporary=False, parent=None):
        ...
        if self.__temporary is False and len(self.cases) > 0:
    ...
    def __set_logo_info(self):
        ...
        self.logo_label.setToolTip(
            self.translations["LOGO_INFO"].format(
    ...
    def __check_selected_logo(self, file_name):
        ...
        if img_format not in self.logo_formats:
            err_msg = self.translations["ERR_SELECTED_LOGO_FORMAT"].format(
                "/".join(self.logo_formats), img_format
            )
        elif img_width < self.logo_minimum_width:
            err_msg = self.translations["ERR_SELECTED_LOGO_MINIMUM_WIDTH"].format(
                self.logo_minimum_width, img_width
            )
        elif img_main_color[1] != (255, 255, 255) and img_alpha_range == (255, 255):
            err_msg = self.translations["ERR_SELECTED_LOGO_BG_COLOR"].format(
                "/".join(self.logo_background)
            )
        if err_msg != "":
            error_dlg = ErrorView(
                QtWidgets.QMessageBox.Icon.Critical,
                self.translations["CHECK_SELECTED_LOGO"],
                err_msg,
                "",
            )
    ...
    def set_current_cases(self):
        ...
        if self.__temporary is False:
            self.name.lineEdit().setPlaceholderText(
                self.translations["SELECT_CASE_NAME"]
            )
            for case in self.cases:
                self.name.addItem(case["name"], case["id"])
            self.name.setCurrentIndex(-1)
        else:
            self.name.setPlaceholderText(self.translations["SELECT_CASE_NAME"])

    def __set_current_proceeding_type(self):
        self.proceeding_type.clear()
        self.proceeding_type.lineEdit().setPlaceholderText(
            self.translations["SELECT_PROCEEDING_TYPE"]
        )
    ...
    def __set_logo_preview(self, data):
        ...
        self.logo_button.setText(self.translations["SELECT_FULL_LOGO"])
    ...
    def __unset_logo_preview(self):
        ...
        self.logo_button.setText(self.translations["SELECT_EMPTY_LOGO"])
